...
class _AdminDashboardScreenState extends State<AdminDashboardScreen>
  ...
  List<_Activity> get _activities => [
    _Activity(icon: Icons.login_rounded,     iconColor: _green,  title: 'Ana Torres ingresó',         time: 'Hace 2 min'),
    _Activity(icon: Icons.check_circle,       iconColor: _blue,   title: 'Luis Cáceres aprobado',      time: 'Hace 5 min'),
    _Activity(icon: Icons.login_rounded,     iconColor: _green,  title: 'María Salinas ingresó',       time: 'Hace 8 min'),
    _Activity(icon: Icons.qr_code_2_rounded, iconColor: _cyan,   title: 'QR generado · 3 usuarios',   time: 'Hace 12 min'),
    _Activity(icon: Icons.login_rounded,     iconColor: _green,  title: 'Pedro Aguirre ingresó',      time: 'Hace 15 min'),
  ];

  // ── Datos gráfico línea (ingresos por hora) ───────────────────
  final List<FlSpot> _lineSpots = const [
    FlSpot(0, 2), FlSpot(1, 8), FlSpot(2, 18), FlSpot(3, 35),
    FlSpot(4, 55), FlSpot(5, 90), FlSpot(6, 70), FlSpot(7, 25),
  ];
  ...
  final List<String> _barDays       = ['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'];
        ...
        Container(
          ...
          child: Row(
            children: [
              ...
              Text('En línea', style: _ts(10, color: _green, fw: FontWeight.w600)),
            ],
          ),
        ),
        // Avatar
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              Text('RF23 · Estadísticas generales · Gala FIE 2026',
          ...
          child: Row(
            children: [
              ...
              Column(
                ...
                children: [
                  Text('$ingresaron / $totalCapacidad',
  ...
  Widget _buildMetricGrid() {
    final metrics = [
      _MetricData(Icons.people_alt_rounded,   _blue,   '$totalRegistrados', 'Registrados',  'Total en sistema'),
      _MetricData(Icons.check_circle_rounded, _green,  '$aprobados',        'Aprobados',    'Pagos verificados'),
      _MetricData(Icons.schedule_rounded,     _yellow, '$pendientes',       'Pendientes',   'En revisión'),
      _MetricData(Icons.cancel_rounded,       _red,    '$rechazados',       'Rechazados',   'Pagos inválidos'),
      _MetricData(Icons.login_rounded,        _cyan,   '$ingresaron',       'Ingresaron',   'Al evento hoy'),
      _MetricData(Icons.qr_code_2_rounded,   _navy,   '$qrGenerados',      'QR Generados', 'Códigos activos'),
    ];
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Ingresos por hora', style: _ts(15, fw: FontWeight.w700)),
                    Text('Flujo de entrada al evento', style: _ts(11, color: _grey)),
                  ],
                ),
              ),
              Container(
                ...
                child: Text('Hoy', style: _ts(11, color: _green, fw: FontWeight.w600)),
              ),
            ],
          ),
          const SizedBox(height: 20),
                  ...
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (v, _) {
                        ...
                        return Text(_lineLabels[idx], style: _ts(9, color: _grey));
                      },
                    ),
                  ),
                ...
                lineBarsData: [
                  LineChartBarData(
                    ...
                    belowBarData: BarAreaData(
                      show: true,
                      gradient: LinearGradient(
                        colors: [_blue.withValues(alpha: 0.25), _blue.withValues(alpha: 0.0)],
            ...
            children: [
              Expanded(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('Pagos semanales', style: _ts(15, fw: FontWeight.w700)),
                    Text('Aprobados vs Rechazados', style: _ts(11, color: _grey)),
                  ],
                ),
              ),
                ...
                titlesData: FlTitlesData(
                  bottomTitles: AxisTitles(
                    sideTitles: SideTitles(
                      ...
                      getTitlesWidget: (v, _) {
                        final idx = v.toInt();
                        if (idx < 0 || idx >= _barDays.length) return const SizedBox();
  ...
  Widget _buildDonutCard() {
    return _CardWrapper(
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('Distribución de asistentes', style: _ts(15, fw: FontWeight.w700)),
          const SizedBox(height: 20),
          Row(
            children: [
              SizedBox(
                ...
                child: PieChart(
                  PieChartData(
                    ...
                    sections: [
                      PieChartSectionData(
                        value: 157, color: _green, radius: 24,
                        title: '', showTitle: false,
                      ),
                      PieChartSectionData(
                        value: 61, color: _blue, radius: 24,
                        title: '', showTitle: false,
                      ),
                      PieChartSectionData(
                        value: 32, color: _cyan, radius: 24,
                        title: '', showTitle: false,
                      ),
                    ],
                  ),
              ...
              Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  _DonutLegend(color: _green, value: '157', label: 'Ingresaron'),
...
class _MetricData {
  ...
  const _MetricData(this.icon, this.color, this.value, this.label, this.sublabel);
}
        ...
        children: [
          Row(
            children: [
              ...
              const Spacer(),
              Icon(Icons.trending_up_rounded, size: 14, color: AppColors.sentryGrey),
            ],
          ),
          const Spacer(),
          Text(
...
class _Legend extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      ...
      const SizedBox(width: 4),
      Text(label, style: GoogleFonts.outfit(fontSize: 10, color: AppColors.sentryGrey)),
    ]);
  }
}
...
class _DonutLegend extends StatelessWidget {
  ...
  final String value;
  final String label;
  const _DonutLegend({required this.color, required this.value, required this.label});
  ...
  Widget build(BuildContext context) {
    return Row(children: [
      ...
      Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        ...
        Text(
          label,
          style: GoogleFonts.outfit(fontSize: 11, color: AppColors.sentryGrey),
        ),
      ]),
    ]);
  }
